...
export default class YinZhuanZheng extends React.Component {
    ...
    moneyInput() {
        ...
        // 校验金额
        if (Common.judgeEmpty(moneyInputVal)) {
            let alertDict = {
                title: "信息提示",
                msg: "请输入转账金额",
                cancel_text: "取消",
    ...
    infoConfirm() {
        ...
        let accountPwd = that.state.accountPwd;
        // 校验密码
        if (Common.PasswordSmsNumber(accountPwd)) {
            let alertDict = {
                title: "信息提示",
                msg: "请输入6位交易密码",
                success_text: "确认",
            ...
            <div>
                ...
                <div ref="myInput">
                    <List.Group>
                        ...
                        <List title="银行账号余额" description={that.state.detail.usableBalance} />
                ...
                <div ref="myConfirm" style={{ display: "none" }}>
                    <MbankPublicResult Small title="转账金额" money={that.state.transactionAmount} />
                    <WhiteSpace size="md" />
                    ...
                    <WhiteSpace size="lg" />
                    <WingBlank size="md">
                        <Button type="primary" size="default" disabled={!(this.state.accountPwd)} onTap={this.infoConfirm.bind(this)}>确认</Button>
                    </WingBlank>
                </div>
                {/* 交易结果信息 */}
                <div ref="myResult" style={{ display: "none" }}>
                    <MbankPublicResult type="success" title="交易成功" />
                    <WhiteSpace size="md" />
                    <MbankTransferConfirm.Group>
                        <MbankTransferConfirm title="银行账号" content={that.state.detail.bankAccount} />
                        <MbankTransferConfirm title="证券资金账号" content={that.state.detail.financialAccount} />
                        <MbankTransferConfirm title="交易金额" content={that.state.transactionAmount} />
                        <MbankTransferConfirm title="券商名称" content={that.state.detail.BrokerName} />
                    </MbankTransferConfirm.Group>
                    <WhiteSpace size="lg" />
                    <WingBlank size="md">
