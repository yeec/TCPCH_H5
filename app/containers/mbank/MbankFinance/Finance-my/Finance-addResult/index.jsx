...
export default class MbankFinanceAddResult extends React.Component {
    ...
    componentDidMount() {
        ...
        //获取理财购买返回结果
        let resultData = JSON.parse(Common.getSessionData(API.SESSION_FINANCE_PRODUCT_ADD_RESTULT));
        that.setState({
            List: resultData
        });
        ...
        return (
            <div>
                ...
                <MbankTransferConfirm.Group>
                    ...
                    <MbankTransferConfirm title="理财期限(天)" content={this.state.ProductInfo.term+'天'}/>
                    <MbankTransferConfirm title="年化收益率(%)" content={this.state.ProductInfo.propRateMin+'%'}/>
                </MbankTransferConfirm.Group>
                <WhiteSpace size="lg"/>
                <WingBlank size="md">
                    <Button type="primary" size="default" onTap={this.turnToMbankFinanceHome.bind(this)}>完成</Button>
                </WingBlank>
            </div>
        )
    }
