...
export default class AccountQuery extends React.Component {
    ...
    componentDidMount() {
        ...
        //拼接账号picker需要的数组
        let that = this;
        let listdata = JSON.parse(Common.getSessionData(API.SESSION_ACCOUNT_DATA_LIST));

        let list = [];
        let cname = '';
        listdata.returnList.map(function (item, i) {

            if (item.alias == "") {
                cname = listdata.customerName;
            } else {
                cname = item.alias;
            }
            list.push({
    ...
    queryDetail(startDate1, accountValue1, endDate1) {
        ...
        //初始化查询
        $Fetch(API.API_QUERY_ACCOUNT_TRANSACTION, {
            //默认固定上送报文
            reqHead: {
                //场景编码
                sceneCode: "AC04",
