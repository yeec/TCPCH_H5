...
export default class SxcHuoZhuanDing extends React.Component {
    ...
    // 转存账号
    showDemandAccountListBox(value) {
        this.setState({
            demandAccount: value
        })
    }
    ...
    savingConfirm() {
        ...
        //     })
        // }
        if (Number(that.state.moneyInputVal) < 30000.00) {
            // 调取客户弹出框提示信息
            let alertDict = {
                title: "错误提示",
                msg: "尊敬的用户，存入定期金额最少为3万元",
                success_text: "确认"
            }
            Common.showAppDialogAlert(alertDict);
            return false;
    ...
    savingResult() {
        ...
        $Fetch(API.API_SET_HUO_ZHUAN_DING, {
            ...
            if (Common.enciperResult(res.rspHead.returnCode)) {
                ...
                param.balance = Common.returnResult(res.rspHead.returnCode) ? res.rspBody.currentToRegular.balance : '';
                that.setState(param)
                // 隐藏录入页面
                $(that.refs.myConfirm).hide();
                // 显示结果页面
        ...
        return (
            <div>
                ...
                <WhiteSpace size="sm" />
                    ...
                    <WhiteSpace size="sm" />
                    <MbankPublicInputMoney inputid="bankininput" inputName="交易金额" placeholder="请输入交易金额" limitFlag={false} value={this.state.moneyInputVal} finalval={this.setMoneyinputval.bind(this)} />
                    <WhiteSpace size="sm" />
                    <Input.Group>
                        <Input disabled={true} labelNumber={6} value='整存整取'>储种</Input>
                        <Input disabled={true} labelNumber={6} value='三年'>存期</Input>
                    </Input.Group>
                    <WhiteSpace size="lg" />
                    <WingBlank size="lg">
                ...
                <div ref="myConfirm" style={{ display: "none" }}>
                    <div className="savings-zhiququereng-democode">
                        ...
                        <MbankPublicConfirm.Group>
                            ...
                            <MbankPublicConfirm title="转出账号" content={CurrentAccount.acNo} />
                        </MbankPublicConfirm.Group>
                        <WhiteSpace size="sm" />
                        {/* <Input.Group>
                ...
                <div ref="myResult" style={{ display: "none" }}>
                    {
                        Common.returnResult(this.state.returnCode) ?
                            <div className="savings-zhiququereng-democode">
                                ...
                                <WhiteSpace size="sm" />
                                <MbankPublicConfirm.Group >
                                    <MbankPublicConfirm title="随心存定期账号" content={this.state.accountNo} />
                                    <MbankPublicConfirm title="储种" content='整存整取' />
                                    <MbankPublicConfirm title="存期" content='三年' />
                                    <MbankPublicConfirm title="转出账号" content={CurrentAccount.acNo} />
                                    <MbankPublicConfirm title="余额" content={this.state.balance} />
                                </MbankPublicConfirm.Group>
                            </div>
                            :
