...
export default class Calendar extends React.Component {
  constructor(props, context) {
    super(props, context);
    this.state = {
      current_year: Parameter.getFullYear(),
      current_month: Parameter.getMonth(),
      current_day: Parameter.getDate(),
      select_year: Parameter.getFullYear(),
      select_month: Parameter.getMonth(),
      select_day: Parameter.getDate(),
  ...
  componentDidMount() {
    ...
    if (year && month && day) {
      let date_num_array = this._initMonthDayNumber(year),
        first_day = Parameter.weekOfMonth(new Date(year, month - 1));
  ...
  componentWillMount() {
    let date_num_array = this._initMonthDayNumber(this.state.current_year),
      first_day = Parameter.weekOfMonth();
  ...
  previousMonth= ()=>  {
    ...
    first_day = Parameter.weekOfMonth(new Date(select_year, select_month));
  ...
  nextMonth = ()=> {
    ...
    this.setState(
      ...
      () => {
        this.props.onNextMonth(select_year, select_month + 1 ,select_day);
      }
    );
  }
  ...
  render() {
    ...
    for (let i = 0; i < month_day; i++) {
      ...
      if (
        ...
        if (
          select_year == history_year &&
          select_month == history_month &&
          history_day == i + 1
        ) {
